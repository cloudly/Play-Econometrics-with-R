...
```
## Warning message: cannot open compressed file 'data/WAGEPAN.rda', probable reason 'No such file or directory'
```
...
```r
# 混合OLS
WAGE_PLM_Pooled <- plm(lwage ~ educ + black + hisp + exper + I(exper2) + 
...
```
## Error: could not find function "plm"
```
...
```
## Error: object 'WAGE_PLM_Pooled' not found
```
...
```r
# 一阶差分
WAGE_PLM_FD <- plm(lwage ~ educ + black + hisp + exper + I(exper2) + 
...
```
## Error: object 'WAGE_PLM_FD' not found
```
...
```r
# 固定效应模型
WAGE_PLM_fixed <- plm(lwage ~ educ + black + hisp + exper + I(exper2) + 
...
```
## Error: object 'WAGE_PLM_fixed' not found
```
...
```r
# 随机效应模型
WAGE_PLM_random <- plm(lwage ~ educ + black + hisp + exper + I(exper2) + 
...
```
## Error: object 'WAGE_PLM_random' not found
```
...
```r
# 组内模型
WAGE_PLM_between <- plm(lwage ~ educ + black + hisp + exper + I(exper2) + 
...
```
## Error: could not find function "pooltest"
```
...
```
## Error: could not find function "plmtest"
```
...
```
## Error: could not find function "pFtest"
```
...
```
## Error: could not find function "phtest"
```
